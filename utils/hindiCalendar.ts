...
export const getHindiDateInfo = () => {
  ...
  const monthIdx = (date.getMonth() + 3) % 12;
  const mahina = months[monthIdx];

  const dayOfMonth = date.getDate();
  const paksh = dayOfMonth <= 15 ? 'शुक्ल पक्ष' : 'कृष्ण पक्ष';
  const tithiNames = ['प्रतिपदा', 'द्वितीया', 'तृतीया', 'चतुर्थी', 'पंचमी', 'षष्ठी', 'सप्तमी', 'अष्टमी', 'नवमी', 'दशमी', 'एकादशी', 'द्वादशी', 'त्रयोदशी', 'चतुर्दशी', dayOfMonth <= 15 ? 'पूर्णिमा' : 'अमावस्या'];
  const tithi = tithiNames[Math.min((dayOfMonth > 15 ? dayOfMonth - 15 : dayOfMonth) - 1, 14)];
