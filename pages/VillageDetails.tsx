    ...
    <div className="flex flex-col min-h-screen bg-transparent">
      <header className="premium-header px-4 pt-8 pb-10 rounded-b-[3.5rem] shadow-xl relative overflow-hidden">
        ...
        <div className="flex flex-col items-center justify-center mb-8 relative z-10">
          <div className="bg-white p-3 rounded-[2.5rem] shadow-2xl border-4 border-indigo-400/30">
            <img src={qrUrl} alt="Village QR" className="w-28 h-28 rounded-2xl" />
          </div>
          <p className="text-[9px] font-black text-white bg-indigo-900/40 px-5 py-1.5 rounded-full uppercase tracking-[0.3em] mt-4 border border-white/20">गाँव की डिजिटल डायरी QR</p>
        </div>

        <div className="bg-white/10 border border-white/20 backdrop-blur-md rounded-full flex items-center px-6 h-14 relative z-10">
           <input type="text" placeholder="गाँव में सदस्य खोजें..." className="bg-transparent outline-none w-full text-sm font-light-custom text-white placeholder-blue-200" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      </header>

      <div className="flex-1 px-4 py-8 space-y-4 pb-24">
        {filteredContacts.map((c) => (
          <div key={c.id} className="bg-white p-5 rounded-[2.25rem] border border-white flex items-center shadow-sm hover:shadow-md transition-all">
            <div className="w-14 h-14 rounded-2xl bg-indigo-50 text-indigo-700 flex items-center justify-center font-heavy-custom text-xl mr-5 shrink-0">{c.name.charAt(0)}</div>
            <div className="flex-1 min-w-0">
              <h4 className="font-heavy-custom text-slate-800 text-base truncate">{c.name}</h4>
            ...
            <div className="flex items-center space-x-2">
              <button onClick={() => handleShare(c)} className="w-10 h-10 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400 active:scale-90"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" /></svg></button>
              <a href={`tel:${c.mobile}`} className="w-14 h-14 bg-indigo-600 rounded-[1.25rem] flex items-center justify-center text-white shadow-xl shadow-indigo-100 active:scale-90">
      ...
      <footer className="bg-white/50 backdrop-blur-md p-8 text-center border-t border-slate-100 print:hidden">
        <div className="flex flex-col items-center space-y-2">
          ...
          <a href={`https://${UI_STRINGS.footerLink}`} target="_blank" rel="noopener noreferrer" className="text-indigo-600 font-heavy-custom text-xs hover:underline decoration-amber-400 decoration-2">
